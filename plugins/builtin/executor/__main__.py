...
from Ubot  import Ubot , Message, config, pool
from Ubot .utils import runcmd

CHANNEL = Ubot .getCLogger()
...
@Ubot .on_cmd("eval", about={
    ...
    'examples': [
        "{tr}eval print('Ubot ')", "{tr}eval -s print('Ubot ')",
...
async def eval_(message: Message):
    ...
    _g, _l = _context(
        context_type, Ubot =Ubot , message=message, replied=message.reply_to_message)
    l_d = {}
    try:
...
@Ubot .on_cmd("term", about={
    'header': "run commands in shell (terminal)",
    'flags': {'-r': "raw text when send as file"},
    'usage': "{tr}term [commands]",
    'examples': "{tr}term echo \"Ubot \""}, allow_channels=False)
@input_checker
...
def _run_coro(future: asyncio.Future, coro: Awaitable[Any],
    ...
    Ubot .loop.call_soon_threadsafe(future.add_done_callback,
    ...
    try:
        ...
        loop.close()
        Ubot .loop.call_soon_threadsafe(lambda: future.done() or future.set_result(None))
