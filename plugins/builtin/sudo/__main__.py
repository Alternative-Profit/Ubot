...
@Ubot.on_cmd("sudo", about={'header': "enable / disable sudo access"}, allow_channels=False)
...
@Ubot.on_cmd("vsudo", about={'header': "view sudo users"}, allow_channels=False)
...
async def add_sudo_cmd(message: Message):
    ...
    if '-all' in message.flags:
        ...
        for c_d in list(Ubot.manager.loaded_commands):
...
@Ubot.on_cmd("vscmd", about={'header': "view sudo cmds"}, allow_channels=False)
