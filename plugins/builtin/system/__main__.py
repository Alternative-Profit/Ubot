...
from Ubot.core.ext import RawClient
from Ubot.utils import extract_entities
from Ubot.utils.exceptions import StopConversation
from .. import system
...
MAX_IDLE_TIME = 300
LOG = Ubot.getLogger(__name__)
CHANNEL = Ubot.getCLogger(__name__)
...
async def restart_(message: Message):
    """ restart Ubot """
    await message.edit("`Restarting Ubot Services`", log=__name__)
...
@Ubot.on_cmd("shutdown", about={'header': "shutdown Ubot :)"}, allow_channels=False)
...
async def die_(message: Message) -> None:
    """ set offline timeout to die Ubot """
    global MAX_IDLE_TIME  # pylint: disable=global-statement
...
@Ubot.on_cmd("listdisabled", about={'header': "List all disabled chats."})
...
@Ubot.on_cmd("convert_usermode", about={
...
@Ubot.on_cmd("convert_botmode", about={
...
async def convert_botmode(msg: Message):
    ...
    try:
        async with Ubot.conversation('botfather') as conv:
            try:
                await conv.send_message('/start')
            except YouBlockedUser:
                await Ubot.unblock_user('botfather')
                await conv.send_message('/start')
            ...
            if 'Sorry' in response.text:
                ...
                await Ubot.promote_chat_member(config.LOG_CHANNEL_ID, username)
