...
from Ubot import Ubot, Message, logging, config, pool, get_collection
...
@Ubot.on_cmd("vdelto", about={'header': "View auto message delete timeout"})
...
@Ubot.on_cmd("vesto", about={'header': "View edit sleep timeout"})
...
async def search(message: Message):
    ...
    data = list(Ubot.manager.loaded_plugins if plugins else Ubot.manager.loaded_commands)
...
@Ubot.on_cmd("logs", about={
    'header': "check Ubot logs",
    'flags': {
...
async def check_logs(message: Message):
    ...
    if '-h' in message.flags and config.HEROKU_APP:
        ...
        await message.client.send_as_file(chat_id=message.chat.id,
                                          text=logs,
                                          filename='Ubot-heroku.log',
                                          caption=f'Ubot-heroku.log [ {limit} lines ]')
    else:
        filename = f"{'loader' if '-l' in message.flags else 'Ubot'}.log"
