from Ubot import Ubot, Message, config


@Ubot.on_cmd("status", about={
...
async def status(message: Message) -> None:
    ...
    if not type_:
        out_str = f"""📊 **--Ubot Status--** 📊
🗃 **Plugins** : `{len(Ubot.manager.plugins)}`
        ✅ **Loaded** : `{len(Ubot.manager.loaded_plugins)}`
        ❎ **Unloaded** : `{len(Ubot.manager.unloaded_plugins)}`
⚔ **Commands** : `{len(Ubot.manager.commands)}`
        ✅ **Loaded** : `{len(Ubot.manager.loaded_commands)}`
        ❎ **Unloaded** : `{len(Ubot.manager.unloaded_commands)}`
⚖ **Filters** : `{len(Ubot.manager.filters)}`
        ✅ **Loaded** : `{len(Ubot.manager.loaded_filters)}`
        ❎ **Unloaded** : `{len(Ubot.manager.unloaded_filters)}`
"""
    elif 'p' in type_:
        if name_:
            if name_ in Ubot.manager.plugins:
                plg = Ubot.manager.plugins[name_]
...
🗂 **Total** : `{len(Ubot.manager.plugins)}`
✅ **Loaded** : `{len(Ubot.manager.loaded_plugins)}`
❎ **Unloaded** : `{len(Ubot.manager.unloaded_plugins)}`
        `{'`,    `'.join((cmd.name for cmd in Ubot.manager.unloaded_plugins))}`
"""
    elif 'c' in type_:
        if name_:
            if not name_.startswith(config.CMD_TRIGGER):
                n_name_ = config.CMD_TRIGGER + name_
            if name_ in Ubot.manager.commands:
                cmd = Ubot.manager.commands[name_]
            elif n_name_ in Ubot.manager.commands:
                cmd = Ubot.manager.commands[n_name_]
            else:
...
🗂 **Total** : `{len(Ubot.manager.commands)}`
✅ **Loaded** : `{len(Ubot.manager.loaded_commands)}`
❎ **Unloaded** : `{len(Ubot.manager.unloaded_commands)}`
        `{'`,    `'.join((cmd.name for cmd in Ubot.manager.unloaded_commands))}`
"""
    elif 'f' in type_:
        if name_:
            if name_ in Ubot.manager.filters:
                flt = Ubot.manager.filters[name_]
...
🗂 **Total** : `{len(Ubot.manager.filters)}`
✅ **Loaded** : `{len(Ubot.manager.loaded_filters)}`
❎ **Unloaded** : `{len(Ubot.manager.unloaded_filters)}`
        `{'`,    `'.join((flt.name for flt in Ubot.manager.unloaded_filters))}`
"""
    else:
...
async def load(message: Message) -> None:
    ...
    type_ = list(message.flags)
    if 'p' in type_:
        found = set(names_).intersection(set(Ubot.manager.plugins))
        if found:
            out = await Ubot.manager.load_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Ubot.manager.commands))
        if found:
            out = await Ubot.manager.load_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Ubot.manager.filters))
        if found:
            out = await Ubot.manager.load_filters(list(found))
            if out:
...
async def unload(message: Message) -> None:
    ...
    if 'p' in type_ and names_:
        found = set(names_).intersection(set(Ubot.manager.plugins))
        if found:
            out = await Ubot.manager.unload_plugins(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Ubot.manager.commands))
        if found:
            out = await Ubot.manager.unload_commands(list(found))
            if out:
        ...
        found = set(names_).intersection(set(Ubot.manager.filters))
        if found:
            out = await Ubot.manager.unload_filters(list(found))
            if out:
...
@Ubot.on_cmd('reload', about={'header': "Reload all plugins"}, allow_channels=False)
async def reload_(message: Message) -> None:
    """ Reload all plugins """
    await message.edit("`Reloading All Plugins`")
    await message.edit(
        f"`Reloaded {await Ubot.reload_plugins()} Plugins`", del_in=3, log=True)


@Ubot.on_cmd('clear_unloaded', about={'header': "clear saved unloaded data in DB"},
               allow_channels=False)
async def clear_(message: Message) -> None:
    """ clear all save filters in DB """
    await message.edit("`Clearing Unloaded Data...`")
    await message.edit(
        f"**Cleared Unloaded** : `{await Ubot.manager.clear_unloaded()}`",
        del_in=3, log=True)