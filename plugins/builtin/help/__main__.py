...
from Ubot import Ubot, Message, config, get_collection
from Ubot.utils import is_command
...
@Ubot.on_cmd("help", about={
    'header': "Guide to use Ubot commands",
...
async def helpme(message: Message) -> None:  # pylint: disable=missing-function-docstring
    plugins = Ubot.manager.loaded_plugins

    if Ubot.has_bot and '-i' in message.flags:
        bot = (await Ubot.bot.get_me()).username
        menu = await Ubot.get_inline_bot_results(bot)
        await Ubot.send_inline_bot_result(
    ...
    if not message.input_str:
        ...
        cat_plugins = Ubot.manager.get_plugins()
...
({size}) <b>Command{'s' if size > 1 else ''}</b>\n\n"""
            ...
            for name, cmd in Ubot.manager.loaded_commands.items():
                for _ in triggers:
                    name = name.lstrip(_)
...
if Ubot.has_bot:
    ...
    def check_owner(func):
        async def wrapper(_, c_q: CallbackQuery):
            if c_q.from_user and c_q.from_user.id in config.OWNER_ID:
                ...
                user_dict = await Ubot.bot.get_user_dict(config.OWNER_ID[0])
                await c_q.answer(
                    f"Only {user_dict['flname']} Can Access this...! Build Your Own @TheUbot 🤘",
                    show_alert=True)
    ...
    async def pm_help_handler(_, msg: Message):
        ...
        commands = Ubot.manager.loaded_commands
    ...
    @Ubot.bot.on_callback_query(filters=filters.regex(pattern=r"\((.+)\)(next|prev)\((\d+)\)"))
    @check_owner
    async def callback_next_prev(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 1:
            buttons = parse_buttons(p_num, cur_pos,
                                    ...
                                    Ubot.manager.get_all_plugins())
        elif len(pos_list) == 2:
            buttons = parse_buttons(p_num, cur_pos,
                                    lambda x: f"🗃 {x}",
                                    Ubot.manager.get_all_plugins()[pos_list[-1]])
        elif len(pos_list) == 3:
    ...
    @Ubot.bot.on_callback_query(filters=filters.regex(pattern=r"back\((.+)\)"))
    @check_owner
    ...
    @Ubot.bot.on_callback_query(filters=filters.regex(pattern=r"enter\((.+)\)"))
    @check_owner
    ...
    async def callback_manage(callback_query: CallbackQuery):
        ...
        if len(pos_list) == 4:
            if is_filter(pos_list[-1]):
                flt = Ubot.manager.filters[pos_list[-1]]
            else:
                flt = Ubot.manager.commands[pos_list[-1]]

            getattr(flt, task)()
            text, buttons = filter_data(cur_pos)

        else:
            plg = Ubot.manager.plugins[pos_list[-1]]
    ...
    @Ubot.bot.on_callback_query(filters=filters.regex(pattern=r"^mm$"))
    @check_owner
    async def callback_mm(callback_query: CallbackQuery):
        await callback_query.edit_message_text(
            "🖥 **Ubot Main Menu** 🖥", reply_markup=InlineKeyboardMarkup(main_menu_buttons()))

    @Ubot.bot.on_callback_query(filters=filters.regex(pattern=r"^chgclnt$"))
    @check_owner
    async def callback_chgclnt(callback_query: CallbackQuery):
        if not Ubot.dual_mode:
    ...
    @Ubot.bot.on_callback_query(filters=filters.regex(pattern=r"refresh\((.+)\)"))
    @check_owner
    ...
    def category_data(cur_pos: str):
        pos_list = cur_pos.split('|')
        plugins = Ubot.manager.get_all_plugins()[pos_list[1]]
    ...
    def plugin_data(cur_pos: str, p_num: int = 0):
        ...
        pos_list = cur_pos.split('|')
        plg = Ubot.manager.plugins[pos_list[2]]
...
"""
    ...
    @Ubot.bot.on_inline_query(group=1)
        ...
        results = [
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    "**Here's how to setup Ubot** 😎"
                ),
                url="https://github.com/UbotTeam/Ubot",
        ...
        if inline_query.from_user and inline_query.from_user.id in config.OWNER_ID:
            ...
            if "msg.err" in inline_query.query:
                ...
                if cmd and err_text and is_command(cmd):
                    bot_username = (await Ubot.bot.get_me()).username
