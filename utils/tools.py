...
_LOG = Ubot .logging.getLogger(__name__)
...
def get_file_id_of_media(message: 'Ubot .Message') -> Optional[str]:
    """ get file_id """
...
async def take_screen_shot(video_file: str, duration: int, path: str = '') -> Optional[str]:
    ...
    ttl = duration // 2
    thumb_image_path = path or join(Ubot .config.Dynamic.DOWN_PATH, f"{basename(video_file)}.jpg")
...
def is_command(cmd: str) -> bool:
    commands = Ubot .Ubot .manager.loaded_commands
    key = Ubot .config.CMD_TRIGGER + cmd
    _key = Ubot .config.SUDO_TRIGGER + cmd
