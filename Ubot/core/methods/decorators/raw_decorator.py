...
from Ubot .plugins.builtin import sudo, system
...
async def _raise_func(r_c: Union['_client.Ubot ', '_client.Ubot Bot'],
...
async def _bot_is_present(r_c: Union['_client.Ubot ', '_client.Ubot Bot'],
...
async def _both_are_admins(r_c: Union['_client.Ubot ', '_client.Ubot Bot'],
                           r_m: RawMessage, is_bot: bool) -> bool:
...
async def _both_have_perm(flt: Union['types.raw.Command', 'types.raw.Filter'],
                          r_c: Union['_client.Ubot ', '_client.Ubot Bot'],
                          r_m: RawMessage, is_bot: bool) -> bool:
    if not await _bot_is_present(r_c, r_m, is_bot):
        return False
    try:
...
class RawDecorator(RawClient):
    ...
    def on_exit(self, callback: Callable[[], Awaitable[Any]]) -> None:
        """ set a callback to calls when the Ubot  is exiting """
    ...
    def _build_decorator(self,
        ...
        def decorator(func: _PYROFUNC) -> _PYROFUNC:
            async def template(r_c: Union['_client.Ubot ', '_client.Ubot Bot'],
                               r_m: RawMessage) -> None:
                ...
                if RawClient.DUAL_MODE and (
                    flt.check_client or (
                        ...
                        and (r_m.from_user.id in config.OWNER_ID or r_m.from_user.id in sudo.USERS))):
                    cond = True
                    async with await _get_lock(str(flt)):
                        ...
                        if cond:
                            if config.Dynamic.USER_IS_PREFERRED:
                                if isinstance(r_c, _client.UbotBot):
                                    return
