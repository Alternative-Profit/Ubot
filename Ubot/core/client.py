...
from Ubot import logging, config
from Ubot.utils import time_formatter
from Ubot.utils.exceptions import UbotBotNotFound
...
class _Module:
    def __init__(self, cat: str, name: str):
        self.cat = cat
        self.name = name
        self._path = f"Ubot.plugins.{cat}.{name}"
...
_Ubot_STATUS = get_collection("Ubot_STATUS")


async def _set_running(is_running: bool) -> None:
    await _Ubot_STATUS.update_one(
        {'_id': 'Ubot_STATUS'},
...
async def _is_running() -> bool:
    if config.ASSERT_SINGLE_INSTANCE:
        data = await _Ubot_STATUS.find_one({'_id': 'Ubot_STATUS'})
        if data:
...
async def _wait_for_instance() -> None:
    ...
    while await _is_running():
        _LOG.info("Waiting for the Termination of "
                  f"previous Ubot instance ... [{timeout} sec]")
        time.sleep(timeout)
...
class _AbstractUbot(Methods, RawClient):
    ...
    async def _load_plugins(self) -> None:
        ...
        _MODULES.clear()
        base = os.path.join("Ubot", "plugins")
    ...
    def __eq__(self, o: object) -> bool:
        return isinstance(o, _AbstractUbot) and self.id == o.id

    def __hash__(self) -> int:  # pylint: disable=W0235
        return super().__hash__()


class UbotBot(_AbstractUbot):
    """ UbotBot, the bot """
    def __init__(self, **kwargs) -> None:
...
class Ubot(_AbstractUbot):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if config.SESSION_STRING and config.BOT_TOKEN:
            ...
            kwargs['bot'] = UbotBot(bot=self, **kwargs)
    ...
    @property
    def bot(self) -> Union['UbotBot', 'Ubot']:
        """ returns Ubotbot """
        if self._bot is None:
            if config.BOT_TOKEN:
                return self
            raise UbotBotNotFound("Need BOT_TOKEN ENV!")
    ...
    def _get_log_client(self) -> _AbstractUbot:
        return self.bot if self.has_bot else self

    async def _log_success(self) -> None:
        # pylint: disable=protected-access
        await self._get_log_client()._channel.log("```Ubot started successfully```")

    async def _log_exit(self) -> None:
        # pylint: disable=protected-access
        await self._get_log_client()._channel.log("```Exiting Ubot ...```")
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        def _handle(num, _) -> None:
            _LOG.info(f"Received Stop Signal [{signal.Signals(num).name}], Exiting Ubot ...")
